...
func (this *LFUCache) Put(key int, value int)  {
    ...
    if node, ok := this.cache[key]; !ok {
        ...
        // key 存在 先从节点所在的双链表移除节点 添加到下一频率的表中
        this.remove(node)
...
func main() {
        ...
        obj.Put(1,1)
        obj.Put(2,2)
        fmt.Println(obj.Get(1))   // 返回 1
        obj.Put(3,3)                // 去除 key 2
        fmt.Println(obj.Get(2))     // 返回 -1 (未找到key 2)
        fmt.Println(obj.Get(3))       // 返回 3
        obj.Put(4,4)                // 去除 key 1

        fmt.Println(obj.Get(1))         // 返回 -1 (未找到 key 1)
        fmt.Println(obj.Get(3))          // 返回 3
